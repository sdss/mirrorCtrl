...
from mirrorCtrl.mirrors import mir25mSecOldModel, mir25mSec
...
def convertSecOrientToNew(orient):
    ...
    """
    encPos = mir25mSecOldModel.encoderMountFromOrient(convOrient2MMRad(orient))
    _orient = mir25mSec.orientFromEncoderMount(encPos[:5], initOrient=convOrient2MMRad(orient))
...
def convertSecOrientToOld(orient):
    ...
    Conversion is based on the modeled secondary mirror geometry (old vs new)
    """
    encPos = mir25mSec.encoderMountFromOrient(convOrient2MMRad(orient))
    _orient = mir25mSecOldModel.orientFromEncoderMount(encPos[:5], initOrient=convOrient2MMRad(orient))
    newOrient = convOrient2UMArcsec(_orient)
    return newOrient
...
if __name__ == "__main__":
    fromDir = os.path.join(os.environ["TCC_25MDATA_DIR"], "instWithOldMirCoeffs")
    toDir = os.path.join(os.environ["TCC_25MDATA_DIR"], "inst")
    # batchConvert(fromDir = fromDir, toDir=toDir)
    inPath = os.path.join(fromDir, "default.dat")
    outPath = os.path.join(toDir, "default.dat")
    FileConverter(inPath, outPath)
